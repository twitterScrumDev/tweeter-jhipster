(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('tweeterApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-tweeterApp-error');
                var entityKey = httpResponse.headers('X-tweeterApp-params');
                if (errorHeader) {
